...
export default function CheckpointEdit() {
  ...
  useEffect(() => {
    // Prefill form by fetching department and locating the checkpoint
    const loadCheckpoint = async () => {
      if (!deptId) return; // rely on localStorage fallback
      try {
        const token = JSON.parse(localStorage.getItem("useraccesstoken"))?.accessToken;
        const res = await axios.get(`${API_BASE}/departments/${deptId}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Ensure dept/op context is available
      if (!deptId || !opId) {
      ...
      if (!token) {
        showAlert({ title: 'Authentication', message: 'You must be logged in to perform this action.', type: 'error' });
        setLoading(false);
        return;
      }
      ...
      const url = `${API_BASE}/departments/${deptId}/operations/${opId}/checkpoints/${chkId}`;
      console.debug('CheckpointEdit -> PUT', url, { form });
