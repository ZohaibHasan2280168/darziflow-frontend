...
export default function UpdateDepartments() {
  ...
  const [form, setForm] = useState({
    name: "",
    description: "",
    departmentHead: "", // Hum yahan sirf ID store karenge
    status: "ACTIVE",
    operations: [],
  });
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      try {
        // 1. Fetch Available Heads for Dropdown
        const headsRes = await axios.get(`${API_URL}/users/available-department-heads`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        setAvailableHeads(headsRes.data.data || []);

        // 2. Fetch Current Department Data
        const deptRes = await axios.get(`${API_URL}/departments/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`${API_URL}/departments/${id}`, requestBody, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  if (fetching) return <div style={{color: 'white', padding: '20px'}}>Loading Department Details...</div>;
