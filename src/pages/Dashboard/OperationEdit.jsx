...
export default function OperationEdit() {
  ...
  useEffect(() => {
    // Prefill form by fetching department and locating the operation (if deptId known)
    const loadOperation = async () => {
      ...
      try {
        ...
        if (!token) return;
        const res = await axios.get(`${API_BASE}/departments/${dept}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = `${API_BASE}/departments/${deptId}/operations/${opId}`;
      console.debug('OperationEdit -> PUT', url, { form });
