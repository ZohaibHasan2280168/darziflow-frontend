...
import { FiX, FiLayers, FiInfo, FiUser } from "react-icons/fi";
...
const EditOrderModal = ({ isOpen, onClose, orderData, onUpdateSuccess }) => {
  ...
  // Jab bhi orderData change ho (ya modal khule), local state update karein
  useEffect(() => {
    if (orderData) {
      setFormData({
        name: orderData.name || "",
        ...
        clientName: orderData.clientName || orderData.clientId?.name || "",
        clientEmail: orderData.clientEmail || orderData.clientId?.email || "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`${API_URL}/${orderData._id}/metadata`, formData, {
        headers: { Authorization: `Bearer ${token}` },
      });

      alert("📝 Order Updated Successfully!");
      onUpdateSuccess(); // Parent component ko batayein ke data refresh kare
        ...
        <div className="modal-header">
          <div className="modal-title">
            ...
            <div>
              ...
              <p>Modify the details of this workflow.</p>
            </div>
          </div>
